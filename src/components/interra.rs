...
pub struct InterraTcpClient {
    ...
    token: RwLock<String>, // todo consider switching to std::sync::RwLock for improved performance in data
}
...
impl InterraTcpClient {
    async fn establish() -> Result<(BufWriter<OwnedWriteHalf>, BufReader<OwnedReadHalf>, String)> {
        ...
        // writer
        //     .write(format!(
        //         "{{'data':{{'action':2,'info':'','token':'','type':2}},'meta':{{'authID':{token},'content_type':null,'error':null,'errorCode':null,'flags':null,'requestType':47,'scheme':null,'serverDateTime':null,'server_version':null,'version':null}}}}\n").as_bytes())
        //     .await?;
        // writer.flush().await?;
    ...
    // pub async fn subscribe_to_room_info(&self, room_id: u16, device_type: DeviceType, subscribe: bool) -> Result<()> {
    //     self.request(None, 19, Some(&format!("[{{'data': {room_id}, 'id': {}, 'value': {subscribe}}}]", device_type.subscription_id()))).await?;
    //     Ok(())
    // }
    ...
    // hardcoded my room because well uhh..each ac item is a different device w/ different id....
    ...
    // will lock until aborted...
    // pub fn log_output(&self) -> JoinHandle<Result<()>> {
    //     let stream = self.stream.clone();
    //     tokio::spawn(async move {
    //         let mut lock = stream.lock().await;  // todo reader will be unusable after this...
    //         let mut out = String::new();
    //
    //         loop {
    //             lock.read_line(&mut out).await?;
    //             println!("{}", out);
    //             out.clear();
    //         }
    //     })
